...
export async function PhotoShowcase({ heading }: { heading: string }) {
  const slides = (await fetchCarouselSlides()).filter((slide) => !/\.(?:mp4|mov|webm)(?:$|[?#])/i.test(slide.url));
  const photos = slides.length ? slides.slice(0, 3) : [{ id: "lawn", url: "/hero-bg.jpg", title: "Nieto Green Care LLC", description: null, location: null }];
