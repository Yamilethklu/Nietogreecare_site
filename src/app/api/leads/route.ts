...
export async function POST(request: Request) {
  ...
  const d=parsed.data; const price=calculateLawnQuote(d.quoteOptions).price; const supabase=getSupabaseAdminClient();
  ...
  const {data:lead,error}=await supabase.from("leads").insert({
    ...
    customer_name:d.customerName,customer_phone:d.customerPhone,customer_email:d.customerEmail||null,details:d.details,additional_notes:d.additionalNotes,payment_method:d.paymentMethod,final_price:price
  }).select().single();
