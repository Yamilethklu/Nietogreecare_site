...
import { ArrowLeft, CalendarDays, Check, Map, Ruler, Send, ShieldCheck } from "lucide-react";
...
export default function QuotePage() {
  ...
  const [hydrated, setHydrated] = React.useState(false); const [mapsReady, setMapsReady] = React.useState(false); const [errors, setErrors] = React.useState<Record<string, string>>({}); const [sending, setSending] = React.useState(false);
  ...
  React.useEffect(() => {
    ...
    const g = window.google.maps; const map = new g.Map(mapNode.current, { center: { lat: store.latitude ?? 30.2672, lng: store.longitude ?? -97.7431 }, zoom: 19, mapTypeId: "satellite", streetViewControl: false, mapTypeControl: false, fullscreenControl: false });
    ...
    const savePolygon = (polygon: any) => { const points = polygon.getPath().getArray().map((point: any) => ({ lat: point.lat(), lng: point.lng() })); const areaSqFt = g.maps.geometry.spherical.computeArea(polygon.getPath()) * 10.7639104; store.setMeasurement(buildMeasurement(points, areaSqFt, store.measurement?.depthInches ?? 2, map.getZoom())); };
    ...
    return () => { g.event.removeListener(overlayListener); manager.setMap(null); };
  ...
  if (store.submitted) return <Confirmation isEs={isEs} onReset={store.reset} />;
  ...
  return <main className="container max-w-5xl py-8 sm:py-12"><Link className="inline-flex items-center gap-2 text-sm text-ink-300 hover:text-gold-200" href="/"><ArrowLeft className="size-4" />{BUSINESS.name}</Link><div className="mt-7 grid gap-7 lg:grid-cols-[1fr_300px]"><Card><CardHeader><p className="ngc-eyebrow">{isEs ? "Solicitud paso a paso" : "Request wizard"}</p><CardTitle>{isEs ? "Solicite su servicio" : "Request your service"}</CardTitle><Progress value={(store.step / TOTAL_STEPS) * 100} /><p className="text-xs text-ink-400">{isEs ? `Paso ${store.step} de ${TOTAL_STEPS}` : `Step ${store.step} of ${TOTAL_STEPS}`}</p></CardHeader><CardContent className="space-y-6">
    ...
    {store.step === 2 && <section className="space-y-5"><h2 className="text-2xl text-white">{isEs ? "Medición satelital" : "Satellite measurement"}</h2>{mapsReady ? <><p className="text-sm text-ink-300">{isEs ? "Trace el perímetro de su patio directamente sobre la imagen satelital. Puede mover los puntos para ajustar el área." : "Draw your yard boundary directly on the satellite image. Move points to refine the area."}</p><div ref={mapNode} className="h-[420px] overflow-hidden rounded-xl border border-gold-500/25" /><div className="grid gap-3 sm:grid-cols-3"><Metric label="sq yd" value={measurement ? formatNumber(measurement.areaSqYd, 1) : "—"} /><Metric label="sq ft" value={measurement ? formatNumber(measurement.areaSqFt) : "—"} /><Metric label="yd³" value={measurement ? formatNumber(measurement.estimatedCubicYards, 2) : "—"} /></div><FieldError>{errors.measurement}</FieldError></> : <div className="rounded-xl border border-wood-500/30 bg-wood-950/20 p-5 text-sm text-ink-200"><Map className="mb-3 size-7 text-gold-300" />{isEs ? "No fue posible cargar Google Maps. Verifique la configuración de la API." : "Google Maps could not load. Check the API configuration."}</div>}</section>}
    ...
    {store.step === 5 && <section className="space-y-5"><h2 className="text-2xl text-white">{isEs ? "Confirme su solicitud" : "Confirm your request"}</h2><p className="text-sm text-ink-300">{isEs ? "Enviaremos sus datos, área medida, fecha solicitada y servicios seleccionados a Nieto Green Care LLC." : "We will send your details, measured area, requested date and selected services to Nieto Green Care LLC."}</p><LuxuryCard><p className="font-semibold text-white">{store.address}</p><p className="mt-2 text-sm text-ink-300">{measurement ? `${formatNumber(measurement.areaSqYd, 1)} sq yd · ${formatNumber(measurement.areaSqFt)} sq ft · ${formatNumber(measurement.estimatedCubicYards, 2)} yd³` : "—"}</p><p className="mt-2 text-sm text-ink-300">{store.selectedServices.map((key) => SERVICES.find((service) => service.key === key)?.[isEs ? "nameEs" : "nameEn"] ?? key).join(", ")}</p></LuxuryCard><Button size="lg" className="w-full" onClick={submit} disabled={sending}><Send className="size-5" />{sending ? (isEs ? "Enviando…" : "Sending…") : (isEs ? "Confirmar Solicitud" : "Confirm Request")}</Button></section>}
...
function Metric({ label, value }: { label: string; value: string }) { return <div className="rounded-xl border border-white/10 bg-ink-950/60 p-4 text-center"><p className="text-xl font-semibold text-white">{value}</p><p className="mt-1 text-xs uppercase tracking-widest text-gold-300">{label}</p></div>; }
function Confirmation({ isEs, onReset }: { isEs: boolean; onReset: () => void }) { return <main className="container flex min-h-dvh max-w-2xl items-center py-16"><LuxuryCard className="w-full text-center"><ShieldCheck className="mx-auto size-14 text-gold-300" /><h1 className="mt-5 text-4xl text-white">{isEs ? "Solicitud enviada" : "Request sent"}</h1><p className="mt-5 leading-7 text-ink-100">{isEs ? CONFIRMATION_ES : "Thank you for requesting our services. Your request was sent successfully. Nieto Green Care LLC will review the information and contact you shortly to confirm the final rate and work date."}</p><div className="mt-8 flex justify-center gap-3"><Button asChild variant="outline"><a href={BUSINESS.telHref}>{BUSINESS.phoneDisplay}</a></Button><Button asChild onClick={onReset}><Link href="/">{isEs ? "Cerrar" : "Close"}</Link></Button></div></LuxuryCard></main>; }